...
void test_backward_pass()
{
    ...
    assert(are_tensors_equal(Tensor::create(A->shape(), std::make_shared<std::vector<float>>(A->grad())), expected_grad_A));
    std::cout << "  Gradient of A verified." << std::endl;
    assert(are_tensors_equal(Tensor::create(B->shape(), std::make_shared<std::vector<float>>(B->grad())), expected_grad_B));
    std::cout << "  Gradient of B verified." << std::endl;
    assert(are_tensors_equal(Tensor::create(C->shape(), std::make_shared<std::vector<float>>(C->grad())), expected_grad_C, 1e-3));
    std::cout << "  Gradient of C verified." << std::endl;
    assert(are_tensors_equal(Tensor::create(D->shape(), std::make_shared<std::vector<float>>(D->grad())), expected_grad_D));
    std::cout << "  Gradient of D verified." << std::endl;
    assert(are_tensors_equal(Tensor::create(E->shape(), std::make_shared<std::vector<float>>(E->grad())), expected_grad_E));
...
void test_backward_pass_with_broadcasting_operations()
{
    ...
    assert(are_tensors_equal(Tensor::create(A->shape(), std::make_shared<std::vector<float>>(A->grad())), expected_grad_A));
    std::cout << "  Gradient of A verified." << std::endl;
    assert(are_tensors_equal(Tensor::create(B->shape(), std::make_shared<std::vector<float>>(B->grad())), expected_grad_B, 1e-6));
    std::cout << "  Gradient of B verified." << std::endl;
    assert(are_tensors_equal(Tensor::create(C->shape(), std::make_shared<std::vector<float>>(C->grad())), expected_grad_C, 1e-6));
    std::cout << "  Gradient of D verified." << std::endl;
    assert(are_tensors_equal(Tensor::create(D->shape(), std::make_shared<std::vector<float>>(D->grad())), expected_grad_D, 1e-6));
    std::cout << "  Gradient of E verified." << std::endl;
    assert(are_tensors_equal(Tensor::create(E->shape(), std::make_shared<std::vector<float>>(E->grad())), expected_grad_E, 1e-6));

...
DecoderLayer::DecoderLayer(int embed_dim, int num_heads, int ff_hidden_dim, float dropout_rate)
                                    ...
                                    bool is_training)
{
    ...
    std::shared_ptr<Tensor> masked_attention_output = masked_self_attention_.forward(
        target_input, target_input, target_input, look_ahead_mask);

    std::shared_ptr<Tensor> masked_attention_output_dropped = dropout1_.forward(masked_attention_output, is_training);
    ...
    std::shared_ptr<Tensor> cross_attention_output_dropped = dropout2_.forward(cross_attention_output, is_training);

    std::shared_ptr<Tensor> cross_attention_residual = *layernorm1_output + cross_attention_output_dropped;
    std::shared_ptr<Tensor> layernorm2_output = layernorm2_.forward(cross_attention_residual);

    std::shared_ptr<Tensor> feed_forward_output = feed_forward_.forward(layernorm2_output);

    std::shared_ptr<Tensor> feed_forward_output_dropped = dropout3_.forward(feed_forward_output, is_training);

    ...
    : normalized_shape_(normalized_shape), epsilon_(epsilon)
{
    ...
    std::shared_ptr<std::vector<float>> gamma_data = std::make_shared<std::vector<float>>(normalized_shape_, 1.0f);
    gamma_->set_data(gamma_data);

    std::shared_ptr<std::vector<float>> beta_data = std::make_shared<std::vector<float>>(normalized_shape_, 0.0f);
...
std::shared_ptr<Tensor> LayerNorm::forward(const std::shared_ptr<Tensor> &input)
{
    ...
    {
        throw std::runtime_error("Input tensor's last dimension must match normalized_shape in LayerNorm.");
    }
    ...
    mean_ = Tensor::create(std::vector<int>(input_shape.begin(), input_shape.end() - 1));
    std::shared_ptr<std::vector<float>> mean_data = std::make_shared<std::vector<float>>(outer_dims_elements);

    std::shared_ptr<Tensor> variance = Tensor::create(std::vector<int>(input_shape.begin(), input_shape.end() - 1));
    std::shared_ptr<std::vector<float>> variance_data = std::make_shared<std::vector<float>>(outer_dims_elements);
    ...
    inv_stddev_ = Tensor::create(std::vector<int>(input_shape.begin(), input_shape.end() - 1));
    std::shared_ptr<std::vector<float>> inv_stddev_data = std::make_shared<std::vector<float>>(outer_dims_elements);
    const std::vector<float> &variance_data_const = variance->get_data();

    for (size_t i = 0; i < outer_dims_elements; ++i)
    {
        (*inv_stddev_data)[i] = 1.0f / std::sqrt((variance_data_const)[i] + epsilon_);
    }
    ...
    std::vector<float> &centered_input_data = const_cast<std::vector<float> &>(centered_input_->get_data());
    ...
    for (size_t i = 0; i < outer_dims_elements; ++i)
    {
        ...
        for (size_t j = 0; j < last_dim_size; ++j)
        {
            centered_input_data[start_idx + j] = input_data[start_idx + j] - current_mean;
            output_data[start_idx + j] = centered_input_data[start_idx + j] * current_inv_stddev * gamma_data[j] + beta_data[j];
        }
    }
...
std::shared_ptr<Tensor> LayerNorm::get_gamma()
{
    return gamma_;
}

std::shared_ptr<Tensor> LayerNorm::get_beta()
{
    return beta_;
}

...
Tensor::Ptr MultiHeadAttention::forward(
  ...
  Tensor::Ptr q_reshaped = q_proj->reshape({(int)batchSize, (int)query_sequence_length, _numHeads, head_dim_});
  Tensor::Ptr k_reshaped = k_proj->reshape({(int)batchSize, (int)key_sequence_length, _numHeads, head_dim_});
  Tensor::Ptr v_reshaped = v_proj->reshape({(int)batchSize, (int)key_sequence_length, _numHeads, head_dim_});
  ...
  Tensor::Ptr q_split_heads = q_reshaped->transpose(perm);
  Tensor::Ptr k_split_heads = k_reshaped->transpose(perm);
  Tensor::Ptr v_split_heads = v_reshaped->transpose(perm);

  Tensor::Ptr attention_output_per_head =
  ...
  Tensor::Ptr attention_output_transposed = attention_output_per_head->transpose(inverse_perm);

  // Reshape to (batchSize, query_sequence_length, embedDim)
  Tensor::Ptr attention_output_concat = attention_output_transposed->reshape(
          {(int)batchSize, (int)query_sequence_length, _embedDim});

  Tensor::Ptr final_output = output_proj_.forward(attention_output_concat);
  return final_output;
}

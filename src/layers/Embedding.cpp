    ...
    : vocab_size_(vocab_size), embed_dim_(embed_dim)
{
    ...
    std::shared_ptr<std::vector<float>> weights_data = std::make_shared<std::vector<float>>(vocab_size_ * embed_dim_);
...
std::shared_ptr<Tensor> Embedding::forward(const std::shared_ptr<Tensor> &input_ids)
{
    ...
    {
        throw std::runtime_error("Embedding layer input must be a 2D tensor (batch_size, sequence_length).");
    }

    size_t batch_size = input_shape[0];
    size_t sequence_length = input_shape[1];

    std::shared_ptr<Tensor> output = Tensor::create({(int)batch_size, (int)sequence_length, embed_dim_});
    ...
    for (size_t i = 0; i < batch_size * sequence_length; ++i)
    {
        ...
        if (token_id_float < 0 || token_id_float >= vocab_size_ || std::fmod(token_id_float, 1.0f) != 0.0f)
        {
            throw std::runtime_error("Input token ID out of vocabulary bounds or not an integer in Embedding forward.");
        }
...
std::shared_ptr<Tensor> Embedding::get_weights()
{
    return weights_;
}

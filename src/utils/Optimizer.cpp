  ...
  for (const std::shared_ptr<Tensor> &param : parameters_) {
    if (param) {
      ...
      std::vector<float> &param_data =
          const_cast<std::vector<float> &>(param->get_data());
...
void Adam::step() {
  ...
  if (clip_coef < 1.0f) {
    for (size_t i = 0; i < parameters_.size(); ++i) {
      ...
      std::vector<float> &param_grad_mutable =
          const_cast<std::vector<float> &>(param->get_grad());
  ...
  for (size_t i = 0; i < parameters_.size(); ++i) {
    ...
    std::vector<float> &m_data =
        const_cast<std::vector<float> &>(m_[i]->get_data());
    std::vector<float> &v_data =
        const_cast<std::vector<float> &>(v_[i]->get_data());

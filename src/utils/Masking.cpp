...
{
    std::shared_ptr<Tensor> mask = Tensor::create({sequence_length, sequence_length});
...
std::shared_ptr<Tensor> create_padding_mask(const std::shared_ptr<Tensor> &input_ids, float pad_token_id)
{
    const std::vector<int> &input_shape = input_ids->get_shape();
    if (input_shape.size() != 2)
    {
        throw std::runtime_error("Input IDs for padding mask must be a 2D tensor (batch_size, sequence_length).");
    }

    size_t batch_size = input_shape[0];
    size_t sequence_length = input_shape[1];

    std::shared_ptr<Tensor> mask = Tensor::create({(int)batch_size, (int)sequence_length});
    std::vector<float> &mask_data = const_cast<std::vector<float> &>(mask->get_data());

...
std::shared_ptr<Tensor> MeanSquaredErrorLoss::compute_loss(std::shared_ptr<Tensor> &predictions, std::shared_ptr<Tensor> &targets)
{
...
std::shared_ptr<Tensor> nll_loss(const std::shared_ptr<Tensor>& log_probabilities, const std::shared_ptr<Tensor>& targets) {
    std::shared_ptr<Tensor> loss = Tensor::create(std::vector<int>{1}, std::make_shared<std::vector<float>>(std::vector<float>{0.0f}));
    ...
    if (last_dim_size == 0 || num_elements_log_prob == 0) {
     ...
     if (log_prob_shape.size() < 1) {
         throw std::runtime_error("Log probabilities tensor must have at least 1 dimension for NLLLoss.");
     }

    size_t outer_dims_elements = num_elements_log_prob / last_dim_size;

    if (targets->num_elements() != outer_dims_elements) {
         throw std::runtime_error("Target data size mismatch with log probabilities outer dimensions in NLLLoss.");
    }
    ...
    for (size_t i = 0; i < outer_dims_elements; ++i) {
        ...
        if (target_val < 0 || target_val >= static_cast<float>(last_dim_size) || std::fmod(target_val, 1.0f) != 0.0f) {
             throw std::runtime_error("Target class index is out of bounds or not an integer in NLLLoss.");
        }
...
std::shared_ptr<Tensor> CrossEntropyLoss::compute_loss(std::shared_ptr<Tensor> &predictions, std::shared_ptr<Tensor> &targets)
{

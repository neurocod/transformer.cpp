...
int main()
{
    try
    {
        ...
        ConfigParser config(config_filename);
        std::cout << "Loaded configuration from " << config_filename << std::endl;

        // General Settings
        bool inference_mode = config.getValue<bool>("inference_mode");
        bool load_existing_weights = config.getValue<bool>("load_existing_weights");
        std::string weights_filename = config.getValue<std::string>("weights_filename");
        std::string data_filename = config.getValue<std::string>("data_filename");

        // Model Architecture
        int embed_dim = config.getValue<int>("embed_dim");
        int max_sequence_length = config.getValue<int>("max_sequence_length");
        int num_layers = config.getValue<int>("num_layers");
        int num_heads = config.getValue<int>("num_heads");
        int ff_hidden_dim = config.getValue<int>("ff_hidden_dim");
        float dropout_rate = config.getValue<float>("dropout_rate");
        float pad_token_id = config.getValue<float>("pad_token_id");

        // Training Parameters
        float learning_rate = config.getValue<float>("learning_rate");
        int num_epochs = config.getValue<int>("num_epochs");
        int batch_size = config.getValue<int>("batch_size");
        int input_seq_length = config.getValue<int>("input_seq_length");
        int decoder_seq_length = config.getValue<int>("decoder_seq_length");

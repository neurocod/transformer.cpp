...
// Function to convert tensor of IDs back to string
std::string tensor_to_string(const std::shared_ptr<Tensor>& tensor, const DataLoader& loader) {
    const auto& id_to_char = loader.get_id_to_char_map();
    const auto& data = tensor->get_data();
    std::string text = "";
    for (float id_float : data) {
        int id = static_cast<int>(id_float);
         auto it = id_to_char.find(id);
         if (it != id_to_char.end() && id != 0) {
             text += it->second;
         } else if (id != 0) {
             text += '?';
         }
...
int main()
{
    ...
    int max_generate_length = 100;
    std::string initial_prompt = "ROMEO:";

...
std::shared_ptr<Tensor> string_to_tensor(const std::string &text, const DataLoader &loader, int seq_len)
{
    ...
    return Tensor::create({1, seq_len}, std::make_shared<std::vector<float>>(ids));
}
...
int main()
{
    ...
    bool inference_mode = ConfigParser::getInstance().getValue<bool>("inference_mode");
    bool load_existing_weights = ConfigParser::getInstance().getValue<bool>("load_existing_weights");
    std::string weights_filename = ConfigParser::getInstance().getValue<std::string>("weights_filename");
    std::string data_filename = ConfigParser::getInstance().getValue<std::string>("data_filename");

    // Model Architecture
    int embed_dim = ConfigParser::getInstance().getValue<int>("embed_dim");
    int max_sequence_length = ConfigParser::getInstance().getValue<int>("max_sequence_length");
    ...
    int ff_hidden_dim = ConfigParser::getInstance().getValue<int>("ff_hidden_dim");
    float dropout_rate = ConfigParser::getInstance().getValue<float>("dropout_rate");
    float pad_token_id = ConfigParser::getInstance().getValue<float>("pad_token_id");

    // Training Parameters
    float learning_rate = ConfigParser::getInstance().getValue<float>("learning_rate");
    ...
    int input_seq_length = ConfigParser::getInstance().getValue<int>("input_seq_length");
    int decoder_seq_length = ConfigParser::getInstance().getValue<int>("decoder_seq_length");

    // Inference Parameters
    int max_generate_length = ConfigParser::getInstance().getValue<int>("max_generate_length");
    std::string initial_prompt = ConfigParser::getInstance().getValue<std::string>("initial_prompt");
    ...
    if ((load_existing_weights || inference_mode) && std::filesystem::exists(weights_filename))
    {
        ...
        catch (const std::exception &e)
        {
            ...
            if (inference_mode)
            {
                std::cerr << "Cannot run inference without weights. Exiting." << std::endl;
                return 1;
            }
    ...
    else
    {
        if (inference_mode)
        {
            std::cerr << "Weights file '" << weights_filename << "' not found. Cannot run inference. Exiting." << std::endl;
            return 1;
        }
    ...
    else
    {
        ...
        if (!weights_loaded)
        {
            std::cerr << "Weights were not loaded (or failed to load). Cannot run inference. Exiting." << std::endl;
            return 1;
        }
        ...
        std::shared_ptr<Tensor> current_input_ids = string_to_tensor(initial_prompt, data_loader, current_seq_len);
        std::vector<float> generated_ids = current_input_ids->get_data();

        // Remove padding from initial prompt display if any was added by string_to_tensor
        ...
        for (int i = 0; i < max_generate_length; ++i)
        {
            ...
            std::shared_ptr<Tensor> step_input_tensor = Tensor::create({1, current_seq_len}, std::make_shared<std::vector<float>>(step_input_vec));

            // Encoder input and Decoder input are the same for autoregressive generation
            ...
            std::shared_ptr<Tensor> logits = model.forward(encoder_input, decoder_input, false);
            ...
            {
                std::cerr << "\nError: last_token_index is negative (" << last_token_index << "). current_total_len=" << current_total_len << ". Breaking." << std::endl;
                break;
            }
            ...
            {
                std::cerr << "\nError: Logits offset (" << logits_offset << ") + vocab_size (" << target_vocab_size
                            << ") exceeds logits_data size (" << logits_data.size() << "). last_token_index=" << last_token_index << ". Breaking." << std::endl;
                break;
            }
            ...
            if (predicted_id == -1)
            {
                std::cerr << "\nError: No valid predicted_id found (argmax failed?). Breaking." << std::endl;
                break;
            }
